...
const emptyNogs = require("../input/empty.json");
...
console.red = logRed;
console.green = logGreen;

async function start() {
  let nogs;
  try {
    nogs = require(`../input-parts/${process.argv[2]}`);
    console.green(`Processing: ${process.argv[2]}`);
  } catch (error) {
    console.red(`name ${process.argv[2]} doesn't exist in /input-parts.`);
    return;
  }
  for (const [index, nog] of nogs.entries()) {
    if (
      excludedNogs.includes(nog) ||
      completedNogs.includes(nog) ||
      emptyNogs.includes(nog)
    )
      continue;
    ...
    try {
      ...
      if (isEmpty(obj) === false) {
        ...
        logGreen(`${index + 1}/${nogs.length} - Nog ${nog} saved. Url: ${url}`);
      } else {
        // update empty
        await updateEmpty(nog);

        logRed(`${index + 1}/${nogs.length} - Nog ${nog} no data. Url: ${url}`);
      }
    } catch (error) {
      // log Error
      console.log(error);
